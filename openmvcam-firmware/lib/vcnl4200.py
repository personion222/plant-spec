...
.. class:: VCNL4200(drvsel, ps=True, als=True, address=DEFAULT_I2C_ADDR, clk=400000)
    ...
    '''
    def __init__(self, scl=Pin("P4"), sda=Pin("P5"), ps=True, als=True, address=DEFAULT_I2C_ADDR, clk=100000):
        self.port = SoftI2C(freq=clk, scl=scl, sda=sda)
        self.port.start()
        if ps:
...
.. method:: configure_ambient_light_sensor(conf)
    ...
    def _read_register(self, address):
        data = self.port.readfrom_mem(DEFAULT_I2C_ADDR, address, 2)
        return data[0] + data[1] * 256

    def _write_register(self, address, low_byte, high_byte=0x00):
        data = bytes((low_byte, high_byte))
        self.port.writeto_mem(DEFAULT_I2C_ADDR, address, data)
